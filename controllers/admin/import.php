	...
	public function index() {
		...
		if ($this->input->post('is_submitted') and ! $this->errors->get_error_codes()) {			
			...
			if ( ! $this->errors->get_error_codes() ) {
				...
				if (empty($xml))
				{
					$this->errors->add('form-validation', __('Please confirm you are importing a valid feed.<br/> Often, feed providers distribute feeds with invalid data, improperly wrapped HTML, line breaks where they should not be, faulty character encodings, syntax errors in the XML, and other issues.<br/><br/>import anything has checks in place to automatically fix some of the most common problems, but we can’t catch every single one.<br/><br/>It is also possible that there is a bug in import anything, and the problem is not with the feed.<br/><br/>If you need assistance, please contact support – <a href="mailto:support@soflyy.com">support@soflyy.com</a> – with your XML/CSV file. We will identify the problem and release a bug fix if necessary.', 'pmxi_plugin')); 					
				}
				else{
					wp_redirect(add_query_arg('action', 'element', $this->baseUrl)); die();
				}				
	...
	public function process($save_history = true)
	{
		...
		if ( ! PMXI_Plugin::is_ajax() ) {																
			...
			/*
				...
				The slowdown is partially caused by the need for import anything to read deeper and deeper into the file on each successive iteration. 
				...
				import anything will just split it into 20 chunks, and then read the last chunk from the beginning.
			*/
